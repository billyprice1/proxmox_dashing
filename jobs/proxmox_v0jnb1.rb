...
def get_config
  config_file = File.dirname(File.expand_path(__FILE__)) + '/../../shared/proxmox_dashing/config_v0jnb1.yml'
...
def report_cluster_status(site,auth_params)
  ...
  send_event("v0jnb1_pvecluster", { state: 'critical', message: "Cluster lost quorum!" } ) if cluster_status[0]['quorate'] != 1
  if downhostlist.empty?
    send_event("v0jnb1_pvecluster", { state: 'ok', message: 'Cluster has quorum and all hosts up' } )
  else
    send_event("v0jnb1_pvecluster", { state: 'warning', message: "Host(s) not up:\n #{downhostlist.join(", ")}"} )
  end

  unless ha_status[0]['status'] == "OK"
    send_event("v0jnb1_haservers", { state: 'critical', message: 'HA status is critical' } )
  else
    send_event("v0jnb1_haservers", { state: 'ok', message: 'All HA servers are running' } )
  end
end

def report_total_failure
  send_event("v0jnb1_pvecluster", { state: 'critical', message: "KVM cluster unreachable" } )
  send_event("v0jnb1_haservers", { state: 'critical', message: "KVM cluster unreachable" })
end
...
SCHEDULER.every '20s', :first_in => 0 do |job|
  ...
  send_event("v0jnb1_hosts_and_kernels", { rows: rows } )
